  ...
  <div class="app-container">
    ...
    <div class="head-container">
      <div v-if="crud.props.searchToggle">
        ...
        />
        <el-input-number v-model="crud.query.highRiskCount" placeholder="高风险漏洞数量" style="width: 150px;" class="filter-item" :min="0" @keyup.enter.native="crud.toQuery" />
        <el-input-number v-model="crud.query.mediumRiskCount" placeholder="中风险漏洞数量" style="width: 150px;" class="filter-item" :min="0" @keyup.enter.native="crud.toQuery" />
        <el-input-number v-model="crud.query.lowRiskCount" placeholder="低风险漏洞数量" style="width: 150px;" class="filter-item" :min="0" @keyup.enter.native="crud.toQuery" />
    ...
    <el-dialog :close-on-click-modal="false" :before-close="crud.cancelCU" :visible.sync="crud.status.cu > 0" :title="crud.status.title" width="500px">
      <el-form ref="form" :model="form" :rules="rules" size="small" label-width="100px">
        ...
        <el-form-item label="高风险漏洞数量" prop="highRiskCount">
          <el-input-number v-model="form.highRiskCount" :min="0" style="width: 370px;" />
        </el-form-item>
        <el-form-item label="中风险漏洞数量" prop="mediumRiskCount">
          <el-input-number v-model="form.mediumRiskCount" :min="0" style="width: 370px;" />
        </el-form-item>
        <el-form-item label="低风险漏洞数量" prop="lowRiskCount">
    ...
    <el-table ref="table" v-loading="crud.loading" :data="crud.data" highlight-current-row stripe style="width: 100%" @selection-change="crud.selectionChangeHandler">
      ...
      <el-table-column prop="highRiskCount" label="高风险漏洞数量" width="140">
        <template slot-scope="scope">
          <el-tag type="danger">{{ scope.row.highRiskCount }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column prop="mediumRiskCount" label="中风险漏洞数量" width="140">
        <template slot-scope="scope">
          <el-tag type="warning">{{ scope.row.mediumRiskCount }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column prop="lowRiskCount" label="低风险漏洞数量" width="140">
        <template slot-scope="scope">
...
const defaultForm = { 
  id: null,
  foundTime: new Date().toISOString().split('T')[0],
  highRiskCount: 0,
  mediumRiskCount: 0,
  lowRiskCount: 0
}
  ...
  data() {
    return {
      ...
      rules: {
        ...
        highRiskCount: [
          { required: true, message: '请输入高风险漏洞数量', trigger: 'blur' },
          { type: 'number', message: '数量必须为数字', trigger: 'blur' }
        ],
        mediumRiskCount: [
          { required: true, message: '请输入中风险漏洞数量', trigger: 'blur' },
          { type: 'number', message: '数量必须为数字', trigger: 'blur' }
        ],
        lowRiskCount: [
          { required: true, message: '请输入低风险漏洞数量', trigger: 'blur' },
          { type: 'number', message: '数量必须为数字', trigger: 'blur' }
        ]
      },
    }
